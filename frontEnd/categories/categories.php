<?php
    // 1. التحقق من وجود parameter "cat" في الرابط
    $category = isset($_GET['cat']) ? $_GET['cat'] : 'categorieCasques';

    // تحويل الاسم إلى التنسيق المستخدم في JSON إذا كان مختلفاً
    $category_mapping = [
        'Casque' => 'categorieCasques',
        'Gants' => 'categorieGants',
        'Corps' => 'categorieCorps',
        'Pieds' => 'categoriePieds',
        'Intervention' => 'categorieIntervention',
        'Incendie' => 'categorieIncendie',
        'Sapeurs-pompiers' => 'categorieSapeursPompiers'
    ];
    ...
    if (!is_readable($json_file)) {
        die("Le fichier products.json n'est pas accessible en lecture");
    }

    // 3. قراءة ملف JSON وتحويله إلى array
    $json_data = file_get_contents($json_file);
    if ($json_data === false) {
        die("Impossible de lire le fichier products.json");
    }
    ...
    if (!isset($data['categories'])) {
        die("La structure du fichier JSON n'est pas correcte");
    }

    // إضافة هذا للتحقق من محتوى JSON
    // echo "<pre>";
    // print_r($data);
    // echo "</pre>";

    // 4. التحقق من أن الكاتيجوري موجودة
    if (!isset($data['categories'][$category])) {
        // إذا لم تكن الفئة موجودة، نعرض رسالة أكثر وضوحاً
        echo "<div class='error-message'>";
        echo "La catégorie demandée n'est pas encore disponible.";
        echo "<br>Veuillez consulter nos catégories disponibles :";
        echo "<ul>";
        foreach ($data['categories'] as $available_category => $products) {
            echo "<li>" . str_replace('categorie', '', $available_category) . "</li>";
        }
        echo "</ul>";
        echo "</div>";
        exit;
